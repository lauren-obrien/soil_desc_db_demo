---
title: "New Zealand Soil Description Manual 2024 - Demonstration database - DuckDB"
...
DuckDB has a Spatial extension but we will not be using it as its overkill for this application (and still under active development at the time of writing).
...
A naming convention is applied to the tables to clearly distingish data-storing tables ('dat\_\*') from support tables containing code lists ('cod\_\*').

## Sites

From [Appendix D.3](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-siteover)
...
CREATE TABLE IF NOT EXISTS dat_sites (
  ...
  dt_site TIMESTAMP CHECK (dt_site <= current_date()), 
...
CREATE TABLE IF NOT EXISTS dat_locations (
 ...
 /* The following assumes EPSG:2193 is in use */
  val_site_coordinate_x DECIMAL(10, 2) NOT NULL 
      CHECK(val_site_coordinate_x >= 1089350 
        AND val_site_coordinate_x <= 4162860),
  val_site_coordinate_y DECIMAL (10, 2) NOT NULL 
  ...
  amt_error_elevation_m DECIMAL(6, 2) CHECK (amt_error_elevation_m >= 0),
...
CREATE TABLE IF NOT EXISTS dat_landscape (
  ...
  amt_lscape_slope_med_d DECIMAL(6, 2) CHECK (amt_lscape_slope_med_d BETWEEN 0 AND 90),
  amt_lscape_slope_min_d DECIMAL(6, 2) CHECK (amt_lscape_slope_min_d BETWEEN 0 AND 90),
  amt_lscape_slope_max_d DECIMAL(6, 2) CHECK (amt_lscape_slope_max_d BETWEEN 0 AND 90)
  );
...
From [Appendix D.4](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-surface)
...
CREATE TABLE IF NOT EXISTS dat_site_surface (
  ...
  amt_depth_water_cm DECIMAL(6, 2) CHECK (amt_depth_water_cm >= 0),
...
CREATE TABLE IF NOT EXISTS dat_surface_cover (
  ...
  amt_surface_cover_p DECIMAL(6, 2) CHECK (amt_surface_cover_p BETWEEN 0 AND 100)
  );
...
From [Appendix D.5](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-horizono)
...
CREATE TABLE IF NOT EXISTS dat_horizon (
  ...
  amt_boundary_width_cm DECIMAL(6, 2) CHECK (amt_boundary_width_cm >= 0),
...
From [Appendix D.6](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-horizona)
...
CREATE TABLE IF NOT EXISTS dat_horizon_arch (
  ...
  amt_rock_frags_p DECIMAL(6, 2) CHECK (amt_rock_frags_p BETWEEN 0 and 100),
  amt_other_frags_p DECIMAL(6, 2) CHECK (amt_other_frags_p BETWEEN 0 and 100),
...
From [Appendix D.7](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-colour)
...
CREATE TABLE IF NOT EXISTS dat_horizon_matrix_colours (
  ...
  catu_colour_moisture VARCHAR REFERENCES cod_colour_moistures(cato_colour_moisture)
  );
...
From [Appendix D.8](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-texture)
...
From [Appendix D.9](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-secf)
...
From [Appendix D.10](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-cons)
...
CREATE TABLE IF NOT EXISTS dat_consistence_tests (
  ...
  cato_sensitivity VARCHAR REFERENCES cod_sensitivities(cato_sensitivity),
  cato_plasticity VARCHAR REFERENCES cod_plasticities(cato_plasticity),
  cato_stickiness VARCHAR REFERENCES cod_stickinesses(cato_stickiness),
...
From [Appendix D.11](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-fts)
...
From [Appendix D.12.2](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-names)
...
From [Appendix D.12.1](https://manaakiwhenua.github.io/NZ_soildesc_2024/ap_03_database_design.html#sec-erd-nzsc)
...
CREATE TABLE IF NOT EXISTS dat_nzsc (
  ...
  /* family */
  catu_nzsc_fam_profile_material VARCHAR REFERENCES cod_nzsc_fam_profile_materials(catu_nzsc_fam_pm),
...
# returns a table describing all the tables in the database
descr <- dbGetQuery(con, 'PRAGMA show_tables_expanded')

```

## Write blank

Save this database definition as an on-disk \*.duckdb file.

Can't be done in one step, so:

```{r}
#| eval: false

if(!dir.exists('template')) { dir.create('template') }

# export database to template folder
dbExecute(con, paste0("EXPORT DATABASE '", 
                      file.path(getwd(), 'template'), "'")) 

# close out in-mem connection
dbDisconnect(con)
```
...
# make and connect to a new blank ondisk file
con2 <- dbConnect(duckdb(dbdir = file.path(getwd(), 'template_nzsd.duckdb')))

# import the schema
dbExecute(con2, 
          paste0("IMPORT DATABASE '", file.path(getwd(), 'template'), "'"))
